...
source = ColumnDataSource(data=dict(name=[], price=[], rating=[], review_count=[], 
	log_review=[], color=[], size=[], score=[], distance = []))
...
hover = HoverTool(tooltips=[
    ...
    ("Price", "@price"),
    ("Distance", "{0} miles".format("@distance")),
])
...
def select_data():
	...
	restaurant_meta = pd.DataFrame(results, columns=['name', 'price', 'rating', 'review_count', 'distance'])
...
print(autoload_server(url="http://localhost:5006/main"))
